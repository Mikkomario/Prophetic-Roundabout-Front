...
export class Dict extends Iterable {
	// Constructor
	...
	// Implemented
	iterator() { return this._pairs.iterator(); }
	...
	filterWith(f, builder = new VectorBuilder()) { return super.filterWith(functionForPairs(f), builder) }
	mapWith(f, builder = new VectorBuilder()) { return super.mapWith(functionForPairs(f), builder) }
	flatMapWith(f, builder = new VectorBuilder()) { return super.flatMapWith(functionForPairs(f), builder) }
	fattenWith(builder = new VectorBuilder) { return super.flattenWith(functionForPairs(f), builder) }
	async asyncMapWith(f, builder = new VectorBuilder()) { return await super.asyncMapWith(functionForPairs(f), builder) }
	...
	get flatten() { return new Dict(flattenWith()) }
	...
	filter(f) { return new Dict(this.filterWith(f)) }
	filterNot(f) { 
		const pairF = functionForPairs(f);
		return this.filter(a => !pairF(a)) 
	}
	map(f) { return this._dictOrVector(this.mapWith(f)); }
	flatMap(f) { return this._dictOrVector(this.mapWith(f)); }
	async asyncMap(f) { return this._dictOrVector(await this.asyncMapWith(f)) }
	// Only maps keys
